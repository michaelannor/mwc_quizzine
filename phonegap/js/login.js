...
function login() {
  var theUrl="http://cs.ashesi.edu.gh/class2016/michael-annor/mwcfinal/response.php?cmd=27&username="+usernameinput.value+"&password="+passwordinput.value;
    ...
    if(obj.result==1){					//check result
      ...
      // $("#simulateClick").trigger("click");
        ...
        window.plugins.toast.showLongBottom('Unable to login! Check Credentials', function(a){console.log('toast success: ' + a)}, function(b){alert('toast error: ' + b)});
    }
}

function signup() {
  var theUrl="http://cs.ashesi.edu.gh/class2016/michael-annor/mwcfinal/response.php?cmd=6&user="+addnewusername.value+"&pass="+addnewpassword.value;
...
if(obj1.result==1){
  ...
  window.plugins.toast.showLongBottom('Unable to add user! :()', function(a){console.log('toast success: ' + a)}, function(b){alert('toast error: ' + b)});
}
